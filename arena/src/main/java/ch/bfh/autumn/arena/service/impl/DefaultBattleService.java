package ch.bfh.autumn.arena.service.impl;
...
public class DefaultBattleService implements BattleService {
  ...
  public String battle(Party challengeeParty, Party challengerParty) {
    ...
    LOG.info("Party '" + challengeeParty.getName() + "' fights against party '" + challengerParty
        .getName() + "'.");
    ...
    while (true) {
      ...
      LOG.info(
          "Party '" + challengeeParty.getName() + "' has " + challengees.size() + " members left.");
      LOG.info(
          "Party '" + challengerParty.getName() + "' has " + challengers.size() + " members left.");

      // check if a party has already lost (no members left), return the winners party name
      if (challengees.isEmpty()) {
        LOG.info("Party '" + challengerParty.getName() + "' wins this battle in " + roundCount
            + " rounds.");
        return challengerParty.getName();
      }

      if (challengers.isEmpty()) {
        LOG.info("Party '" + challengeeParty.getName() + "' wins this battle in " + roundCount
            + " rounds.");
  ...
  private Hero duel(Hero challengee, Hero challenger) {
    ...
    LOG.info(
        "------------------------------------------------------------------------------------------------------------");
    LOG.info(
        attacker.getName() + " (ATK:" + attacker.getAtk() + ", DEF:" + attacker.getDef() + ") with "
            + f.format(attacker.getHp()) + " hp left fights against " + defender.getName()
            + " (ATK:" + defender.getAtk() + ", DEF:" + defender.getDef() + ") with " + f
            .format(defender.getHp()) + " hp left.");
  ...
  private void round(Hero attacker, Hero defender) {
    ...
    defenderHp -= harm;
    LOG.info("Defending " + defender.getName() + " has " + f.format(Math.max(0, defenderHp))
        + " health points left.");
